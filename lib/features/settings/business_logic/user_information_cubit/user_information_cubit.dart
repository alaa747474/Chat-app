...
class UserInformationCubit extends Cubit<UserInformationState> {
  UserInformationCubit(this._userInformationRepository)
      : super(UserInformationInitial());
  final UserInformationRepository _userInformationRepository;
  getProfilePictureUrl({required File file}) {
    _userInformationRepository
        .getProfilePictureUrl(file: file)
        .then((imageUrl) {
  ...
  saveUserData({
    ...
    UserModel userModel = UserModel(
        name,
        email,
        (state as UserProfilePicLoaded).imageUrl,
        FirebaseAuth.instance.currentUser!.phoneNumber.toString(),
        FirebaseAuth.instance.currentUser!.uid);
        FirebaseAuth.instance.currentUser!.updatePhotoURL((state as UserProfilePicLoaded).imageUrl);
